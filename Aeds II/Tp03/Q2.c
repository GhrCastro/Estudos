...
void poke_inserir(pokemon pokemon, int pos, poke_list *lista){
    int i;
    if(lista->n >= MAXTAM_LISTA || pos<0 || pos>lista->n ){
        printf("Erro ao inserir!");
        if(lista->n >= MAXTAM_LISTA){
            printf("\n lista->n >= MAXTAM_LISTA");
        }
...
char* poke_remover_inicio(poke_list *lista){
    ...
    char* removed = strdup(lista->array[0].name);
...
char* poke_remover(int pos, poke_list *lista){
    ...
    char* removed = strdup(lista->array[pos].name);
    for(      i = pos; i < lista->n - 1; i++){
        lista->array[i] = lista->array[i + 1];
    }
    lista->n-=1;
    return removed;
