...
void poke_start(poke_circ*);
void poke_inserir(pokemon ,poke_circ*);
char* poke_remover(poke_circ*);
void poke_mostrar(poke_circ*);
void poke_operar(char*,pokemon,poke_circ*);
...
void poke_start(poke_circ *lista){
    lista->inicio = 0;
    lista->fim = 0;
    lista->tamanho = 0;
}

bool fila_cheia(poke_circ *fila){
    return fila->tamanho == TAMANHO_FILA;
}

bool fila_vazia(poke_circ *fila){
    return fila->tamanho ==0;
}


void poke_inserir(pokemon poke, poke_circ *lista){
    int i;
    if(fila_cheia(lista)){
       lista->inicio = (lista->inicio+1) % TAMANHO_FILA;
       lista->tamanho--;
    }

    lista->array[lista->fim] = poke;
    lista->fim = (lista->fim + 1) % TAMANHO_FILA;
    lista->tamanho++;

    int soma_capture_rate = 0;
    for(i=0; i<lista->tamanho; i++){
        int indice = (lista->inicio + i) % TAMANHO_FILA;
        soma_capture_rate += atoi(lista->array[indice].capture_rate);
    }

    int media_capture_rate = round((double)soma_capture_rate / lista->tamanho);
    printf("Média: %d\n", media_capture_rate);
    
}


void poke_mostrar(poke_circ *lista){
    int i;

    for(i=0;i<lista->tamanho;i++){
        int indice = (lista->inicio + i) % TAMANHO_FILA;
        printf("[%d] ",i);
        imprimir(lista->array[indice]);
    }
}

void poke_operar(char* operacao, pokemon objeto, poke_circ* lista){
    if(strcmp(operacao,"I")==0){
        poke_inserir(objeto,lista);
    }else if(strcmp(operacao,"R")==0){
        if(!fila_vazia(lista)){
            pokemon removido = lista->array[lista->inicio];
            lista->inicio = (lista->inicio + 1) % TAMANHO_FILA;
            lista->tamanho--;
            printf("(R) %s\n",removido.name);
        }
