...
void poke_start(poke_list*);
void poke_inserir_inicio(pokemon ,poke_list*);
void poke_inserir_fim(pokemon,poke_list*);
void poke_inserir(pokemon, int, poke_list*);
char* poke_remover_inicio(poke_list*);
char* poke_remover_fim(poke_list*);
char* poke_remover(int ,poke_list*);
void poke_mostrar(poke_list*);
void poke_operar(char*,pokemon,int,poke_list*);
...
void poke_inserir_inicio(pokemon pokemon, poke_list *lista){
...
void poke_inserir_fim(pokemon pokemon, poke_list *lista){
    lista->ultimo->prox = nova_celula(pokemon);
    lista-> ultimo = lista->ultimo->prox;
}

int tamanho(poke_list *lista){
    int tamanho = 0;
    celula* i;
...
void poke_inserir(pokemon pokemon, int pos, poke_list *lista){
    int tam = tamanho(lista);
    if(pos<0 || pos>tam){
        printf("Erro ao inserir posicao (%d/ tamanho = %d) invalida!",pos,tam);
    }else if(pos==0){
        poke_inserir_inicio(pokemon,lista);
    }else if(pos==tam){
        poke_inserir_fim(pokemon,lista);
    }else{
        int j;
...
char* poke_remover_inicio(poke_list *lista){
    if(lista->primeiro == lista->ultimo){
...
char* poke_remover_fim(poke_list *lista){
    ...
    char* resp = lista->ultimo->elemento.name;
    lista->ultimo = i;
...
char* poke_remover(int pos, poke_list *lista){
    char* resp;
    int tam = tamanho(lista);
