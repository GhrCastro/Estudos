...
public class Tp04Q05 {
    static class Hash{
        ...
        private int size;
        private int reserva = 9;
        private int posReserva = 0;
        ...
        public Hash (int i){
            this.size = i;
            this.array = new Pokemon[i+reserva];
            for(int j=0;j<i;j++){
                this.array[j] = null;
            }
        }

        public int hashear(String elemento) {
            int hash = 0;
            for (char c : elemento.toCharArray()) {
                hash += (int) c; // Add the ASCII value of each character
            }
        ...
        public boolean inserir(Pokemon x){
            boolean resp = false;
            if(x!=null){
                int pos = hashear(x.getName());
                if(this.array[pos] == null){
                    this.array[pos] = x;
                    resp = true;
                }else if(this.posReserva<this.reserva){
                    this.array[size+posReserva] = x;
                    posReserva++;
                    resp = true;
                }
            }
        ...
        public boolean pesquisar(String name){
            boolean resp = false;
            int pos = hashear(name);
            if(this.array[pos].getName().equals(name)){
                System.out.print("(Posicao: " + pos +") ");
                resp = true;
            }else if(this.array[pos]!=null){
                for(int i=0;i<reserva;i++){
                    if(this.array[size+i].getName().equals(name)){
                        System.out.print("(Posicao: " + (size+i) +") ");
                        resp = true;
                        i = reserva;
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        Hash pokemons= new Hash();//Array dos pokemons que serão pegos pelos ids
        ...
        while(!entrada.equals("FIM")){
            System.out.print("=> "+pesquisados.get(0)+": ");
            if(pokemons.pesquisar(pesquisados.get(0)) == true){
                System.out.println("SIM");
            }else{
                System.out.println("NAO");
